...
inline Vec2 Scene::getTextureCoordinates(const Ray &ray, const int tri_id) const
{
	...
	coordX=alpha*uv[tri_id * 3].x+beta*uv[tri_id * 3+1].x+gamma*uv[tri_id * 3+2].x;
	coordY=alpha*uv[tri_id * 3].y+beta*uv[tri_id * 3+1].y+gamma*uv[tri_id * 3+2].y;
	Vec2 texel(coordX, coordY);
